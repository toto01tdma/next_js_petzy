    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Partner/Owner login - Handle accommodation approval data from response
                // Store approval status and accommodation name from the login response
                const accommodationApproval = data.data.user.accommodationApproval;
                
                if (accommodationApproval) {
                    // Store approval status
                    localStorage.setItem('approvalStatus', accommodationApproval.status || 'DRAFT');
                    
                    // Store accommodation name (prefer Thai name, fallback to English name)
                    const accommodationName = accommodationApproval.accommodationName || 
                                             accommodationApproval.accommodationNameEn || 
                                             'โรงแรมของคุณ';
                    localStorage.setItem('accommodationName', accommodationName);
                    
                    // Store accommodation ID if available
                    if (accommodationApproval.accommodationId) {
                        localStorage.setItem('accommodationId', accommodationApproval.accommodationId);
                    }
                } else {
                    // No accommodation approval data - likely first time user
                    localStorage.setItem('approvalStatus', 'DRAFT');
                ...
                if (approvalStatus === 'APPROVED') {
                    ...
                    // DRAFT, PENDING_REVIEW, or REJECTED - go to data entry
                    router.push('/partner/data-entry');
                }
            } else {
                // Handle error
